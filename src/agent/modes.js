    ...
    },
    {
        ...
        update: async function (agent) {
            if (!this.active && !this.paused) {
                this.active = true; // Set the mode as active when it starts.
                while (this.active && !this.paused) {
                    await skills.stayAwake(agent.bot); // Call the stay awake skill.
                    await new Promise(resolve => setTimeout(resolve, 30 * 60 * 1000)); // Wait for 30 minutes.
                }
                this.active = false; // Reset active status when done or paused.
            }
        }
    }
];
