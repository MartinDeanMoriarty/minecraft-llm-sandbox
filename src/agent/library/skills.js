...
export * from './skills/takeScreenshot.js';
export * from './skills/stayAwake.js';
...
export async function goToPosition(bot, x, y, z, min_distance=2) {
    ...
    bot.pathfinder.setMovements(new pf.Movements(bot));
    const initialDistance = getDistanceToTarget(bot, x, y, z);
    let lastDistance = initialDistance;
    const maxAttempts = 100; // Maximum number of attempts to check if the bot is moving towards the target
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
        await sleep(500); // Wait for half a second before checking again
        const currentDistance = getDistanceToTarget(bot, x, y, z);
        if (currentDistance <= min_distance) {
            log(bot, `You have reached at ${x}, ${y}, ${z}.`);
            return true;
        }
        if (currentDistance >= lastDistance) {
            // If the bot is not making progress, break out of the loop to avoid getting stuck
            break;
        }
        lastDistance = currentDistance;
    }
    log(bot, `Failed to reach ${x}, ${y}, ${z} within ${maxAttempts} attempts. You are stuck. Use !moveAway and try again.`);
    return false;
}
...
export async function useCommand(bot, com) {
    if (!com){
        log(bot, 'Failed. Provide a command like: tp yourname playername.');
        return false
    }
    /**
     ...
     **/
        bot.chat('/' + com);       
        log(bot, `Command ${com} executed!`);
        return true;
} 

export async function jump(bot) {
    /**
     * Jump to greed a player.
     * @param {MinecraftBot} bot, reference to the minecraft bot.
     * @example
    ...
    log(bot, `You just jumped.`);
    return true;
}

export async function lookInDirection(bot, direction) {
    if (!direction){
        log(bot, 'Failed. Provide a direction.');
        return false
    }
    /**
     * Make the bot look in a specific direction.
     * @param {MinecraftBot} bot, reference to the minecraft bot.
     * @param {string} direction, the direction to look at: "north", "east", "south", or "west".
     * @returns {Promise<boolean>} true if the bot has successfully looked in the specified direction, false otherwise.
     * @example
     * await skills.lookInDirection(bot, "north");
     **/
    ...
    switch (direction) {
        case 'north':
            yaw = 0; // North is 0 degrees in Minecraft's yaw system
            break;
        case 'east':
            yaw = 90; // East is 90 degrees in Minecraft's yaw system
            break;
        case 'south':
            yaw = 180; // South is 180 degrees in Minecraft's yaw system
            break;
        case 'west':
            yaw = -90; // West is -90 degrees in Minecraft's yaw system
            break;
        default:
            log(bot, `Invalid direction: ${direction} Use north , east , south or west for direction.`);
            return false;
    }

    bot.setControlState('yaw', yaw);
    log(bot, `Now looking ${direction}.`);
    return true;
}

export async function lookAtPlayer(bot, username) {
    if (!username){
        log(bot, 'Failed. Provide a playername.');
        return false
    }
    /**
     * Look straight at the specified player
     * @param {MinecraftBot} bot, reference to the minecraft bot.
     * @param {string} username, the username of the player.
     * @returns {Promise<boolean>} true, false otherwise.
     * @example
     * await skills.lookAtPlayer(bot, "username");
     **/
    ...
    await bot.lookAt(player.position);   
    log(bot, `You are looking at ${username}.`);
    return true;
}

export async function lookingAt(bot) {
    /**
     * At what is the bot looking.
     * @param {MinecraftBot} bot, reference to the minecraft bot.
     * @example
     * await skills.lookingAt(bot);
     **/
    ...
    if (!block) {    
       log(bot, `You are looking at Air`);
    }
    log(bot, `You are looking at ${block.displayName}`);
    return true;
}