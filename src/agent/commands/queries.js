...
export const queryList = [
    ...
    {
        name: '!getTime',
        description: 'Returns the actual time.',
        perform: async function (agent) {
            return "Time now: " + moment().format('HH:mm');
        }
    },
    {
        name: "!playersOnline",
        description: "Get all the players online.",
        perform: function (agent) {
            let bot = agent.bot;
            
            let res = 'PLAYERS_ONLINE';
            for (const entity of world.getPlayersOnline(bot)) {
                res += `\n- player: ${entity}`;
            }

            if (res == 'PLAYERS_ONLINE') {
                res += ': none';
            }
            return pad(res);
        }
