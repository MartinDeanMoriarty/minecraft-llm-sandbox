...
export const actionsList = [
    ...
    },
    {
        ...
        perform: wrapExecution(async (agent, itemName, num=-1) => {
            await skills.equip(agent.bot, itemName, num=-1);
        })
    },
    {
        name: '!discardItem',
        description: 'Discard/Throw the given item out the inventory.',
        params: {
            'itemName': '(string) The name of the item to equip.',
            'num': '(number) The number of items to discard.'        
        },
        perform: wrapExecution(async (agent, itemName, num) => {
            await skills.discard(agent.bot, itemName, num);
        })
    },
    {
        name: '!smeltItem',
    ...
    {
        name: '!clearFurnace',
        description: 'Clear/Take out items from nearest furnace',
        perform: wrapExecution(async (agent, type) => {           
    ...
    {
        name: '!useCommand',
        description: 'Use a server command, for example !useCommand(tp bot player_name).',
        params: {'command': '(string) The command to use.'},
        perform: wrapExecution(async (agent, command) => {
            await skills.useCommand(agent.bot, command);
        })
    },
    {
        name: '!setReminder',
        description: 'Set a riminder for this session for what ever you need it.',       
        params: {
            'description': '(string) Descipe briefly what you want to be reminded of.',
            'time': 'Time at which to remind, in HH:mm format.'
        },
        perform: wrapExecution(async (agent, description, time) => {
            await skills.setReminder(agent.bot, description, time);
        })
    },    
    {
        name: '!jump',
        description: 'Jump.',
        perform: wrapExecution(async (agent) => {
            return await skills.jump(agent.bot);
        })
    },
    {
        ...
        params: {
            'direction': '(string) Use north , east , south or west for direction. Pauses idle_staring mode.'
        },
        perform: wrapExecution(async (agent, direction) => {
            return await skills.lookInDirection(agent.bot, direction);
        })
    },
    {
        name: '!lookAtPlayer',
        ...
        params: {
            'player_name': '(string) The name of the player to look at. Pauses idle_staring mode.'
        },
        perform: wrapExecution(async (agent, player_name) => {
            return await skills.lookAtPlayer(agent.bot, player_name);
        })
    },
    {
        name: '!lookingAt',
        description: 'At what block you are looking right now. Pauses idle_staring mode.',
        perform: wrapExecution(async (agent) => {
    ...
    {         
        ...
        perform: wrapExecution(async (agent, screenshot_name) => {
            const now = Date.now();
            if (!lastExecutionTime.has('takeScreenshot') || now - lastExecutionTime.get('takeScreenshot') > 30000) {
                await skills.takeScreenshot(agent.bot, screenshot_name);
                lastExecutionTime.set('takeScreenshot', now);
            } else {                
